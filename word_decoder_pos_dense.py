...
def load_word_embedding(embed_file, vocab):
    ...
    embed_vocab = OrderedDict()         #store word to an integer index
    rev_embed_vocab = OrderedDict()     #get word from its index
    embed_matrix = list()               #store embedding of each word in vocab

    embed_vocab['<PAD>'] = 0            #add pas, unk, sos, eos tokens
    ...
    with open(embed_file, "r") as f:
        for line in f:
            ...
            if len(parts) < word_embed_dim + 1:
                continue
            word = parts[0]
            if word in vocab and vocab[word] >= word_min_freq:  #vocab contains freq of words
                vec = [np.float32(val) for val in parts[1:]]    #add embedding to matrix, dicts
                embed_matrix.append(vec)
                embed_vocab[word] = word_idx
                rev_embed_vocab[word_idx] = word
                word_idx += 1

    for word in vocab:                                      #add words not in vocab but have freq above threshold
...
def build_vocab(data, rels, vocab_file, embed_file, pos_vocab):        #return map of word, rev word ,char and embedding matrix
    vocab = OrderedDict()
    char_v = OrderedDict()
    char_v['<PAD>'] = 0
    char_v['<UNK>'] = 1
    ...
    pickle.dump([word_v, char_v, pos_vocab], output)           #store word and char vocab in output file
    output.close()
...
def load_vocab(vocab_file):
    with open(vocab_file, 'rb') as f:
        word_v, char_v, pos_vocab = pickle.load(f)
    return word_v, char_v, pos_vocab
...
def get_data(src_lines, trg_lines,  datatype):      #store data as tuples in sample recordclass
    samples = []
    uid = 1
    src_len = -1
    trg_len = -1
...
def read_data(src_file, trg_file,  datatype):       #read sentence and trilpets
...
def get_relations(file_name):       #get relations 
    rels = []
...
def get_pred_words(preds, attns, src_words):        #get target words from indices
    pred_words = []
...
class POSEmbeddings(nn.Module):
    def __init__(self, tag_len,tag_dim, drop_out_rate):
        ...
        # self.embeddings.weight.data.copy_(torch.from_numpy(tag_embedding_matrix))
        self.dropout = nn.Dropout(drop_out_rate)
...
class Encoder(nn.Module):
    ...
    def forward(self, words_input, src_pos_seq, char_seq,  is_training=False):
        ...
        src_pos_embeds = self.pos_embeddings(src_pos_seq)   

        words_input = torch.cat((words_input,src_pos_embeds, char_feature), -1)
        if enc_type == 'LSTM':
...
class SeqToSeqModel(nn.Module):
    ...
    def forward(self, src_words_seq, src_pos_seq,src_chars_seq, src_mask, trg_words_seq, trg_vocab_mask,  is_training=False):
        ...
        encoder_output = self.encoder(src_word_embeds, src_pos_seq,  src_chars_seq,  is_training)
...
def predict(samples, model, model_id):
    ...
    #    adj = autograd.Variable(adj)
        ...
        with torch.no_grad():
            if model_id == 1:
                outputs = model(src_words_seq,src_pos_tags, src_chars_seq, src_words_mask, trg_words_seq, trg_vocab_mask,
                                False)
...
def train_model(model_id, train_samples, dev_samples, best_model_file):
    ...
    for epoch_idx in range(0, num_epoch):
        ...
        for batch_idx in tqdm(range(0, batch_count)):
            ...
            if model_id == 1:
                outputs = model(src_words_seq, src_pos_tags,src_chars_seq, src_words_mask, trg_words_seq, trg_vocab_mask, True)
