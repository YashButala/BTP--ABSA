...
class Encoder(nn.Module):
	...
	def forward(self, words, chars, pos_seq, dep_seq, loc_seq, adv=None, is_training=False):
		...
		# words_input = torch.cat((src_word_embeds, char_feature, src_pos_embeds, src_dep_embeds), -1)
	...
	for epoch_idx in range(0, num_epoch):
		...
		for batch_idx in tqdm(range(0, batch_count)):
			...
			if use_adv:
				...
				adv_outputs = model(src_words_seq, src_words_mask, src_chars_seq, src_pos_tags, src_dep_tags, trg_words_seq,
...
if __name__ == "__main__":
	...
	embedding_file = os.path.join(src_data_folder, 'cased_glove300.txt')
	update_freq = 1
	wf = 1
	att_type = 2
	# max_dist = 10
