...
def build_tags(file1, file2):
    ...
    for line in f1:
        ...
        tags = nltk.pos_tag([i for i in text if i])
        for t in tags:
...
def get_data1(src_lines,  datatype):
    samples = []
    uid = 1
    for i in range(0, len(src_lines)):
        src_line = src_lines[i].strip()
        src_words = src_line.split()
        sample = Sample(Id=uid, SrcLen=len(src_words), SrcWords=src_words,TrgLen = 0, TrgRels=[], TrgPointers=[])
        samples.append(sample)
        uid += 1
    return samples

def read_data1(src_file,  datatype):
    ...
    for line in reader:
        ...
        # print(line)
    reader.close()
    data = get_data1(src_lines,  datatype)
    return data
...
def get_hc_pred_triples(rel, arg1s, arg1e, arg2s, arg2e, src_words):
    triples = []
    all_triples = []
    out = []
    for i in range(0, len(rel)):
        ...
        pe2 = arg2e[i][e2]
        pr = rel[i][r]
        # print(ps1,pe1,ps2,pe2,pr)
        if ps1<0.6 or pe1<0.6 or ps2<0.6 or pe2<0.6 or pr<0.6:
            # print('her')
            continue
        if (ps1*pe1*ps2*pe2*pr)**(1/float(5))<0.7:
            # print('there')
            continue 
        arg1 = ' '.join(src_words[s1: e1 + 1])
        arg2 = ' '.join(src_words[s2: e2 + 1])
        ...
        triplet = (arg1, arg2, relIdxToName[r])
        all_triples.append(triplet)
        outc = (s1,e1,s2,e2,relIdxToName[r])
        if not is_full_match(triplet, triples):
            triples.append(triplet)
            out.append(outc)
    return triples, all_triples, out
...
def write_hc_res(src,data,preds,outfile):
    reader = open(src)
    pm = 0
    ps = 0
    nm = 0
    ...
    for i in range(0, len(src_lines)):
        text = src_lines[i]
        pred_triples, _ ,out = get_hc_pred_triples(preds[0][i], preds[1][i], preds[2][i], preds[3][i], preds[4][i],
                                        data[i].SrcWords)
        pred_triples_str = []
        # print(out)
        f = 1
        ...
        for j in range(len(out)):
            o = out[j]
            pt = pred_triples[j]
            ap_beg, ap_end, op_beg, op_end, p = o
            ap = pt[0]
            op = pt[1]
            polarity = p
            if polarity=='NEG':
        ...
        if tup=='':
            continue
        if len(out)>1 and f==1 and pm<3000:
            pm += 1
            tupsfile.write(tup[:-3]+'\n')
            ptrfile.write(num[:-3]+'\n')
            sentfile.write(text)
        if len(out)>1 and f==0 and nm<3000:
            nm += 1
            tupsfile.write(tup[:-3]+'\n')
            ptrfile.write(num[:-3]+'\n')
            sentfile.write(text)
        if len(out)==1 and f==1 and ps<3000:
            ps += 1
            tupsfile.write(tup[:-3]+'\n')
            ptrfile.write(num[:-3]+'\n')
            sentfile.write(text)
        if len(out)==1 and f==0 and ns<3000:
            ns += 1
            tupsfile.write(tup[:-3]+'\n')
            ptrfile.write(num[:-3]+'\n')
            sentfile.write(text)
...
class Encoder(nn.Module):
    ...
    def forward(self, words, src_pos_seq,chars, pos_seq,  is_training=False):
...
class Seq2SeqModel(nn.Module):
    ...
    def forward(self, src_words_seq,src_pos_seq, src_mask, src_char_seq, pos_seq, trg_words_seq, trg_rel_cnt, 
        ...
        enc_hs = self.encoder(src_words_seq, src_pos_seq ,src_char_seq, pos_seq, is_training)
...
def predict(samples, model, model_id):
    ...
    for batch_idx in tqdm(range(0, batch_count)):
        ...
        with torch.no_grad():
            if model_id == 1:
                outputs = model(src_words_seq, src_pos_tags,src_words_mask, src_chars_seq, positional_seq, trg_words_seq,
...
if __name__ == "__main__":
    ...
    if job_mode == 'infer':
        ...
        custom_print('\nTest Results\n')
        src_test_file = os.path.join(src_data_folder, 'elec.txt')
        test_data = read_data1(src_test_file,  3)
        ...
        # custom_print('R:', round(r, 3))
        # custom_print('F1:', round(test_acc, 3))
        write_hc_res(src_test_file, test_data,test_preds, os.path.join(trg_data_folder, 'test.out'))
