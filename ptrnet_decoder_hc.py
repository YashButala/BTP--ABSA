...
def read_data(src_file,  datatype):
    ...
    data = get_data(src_lines,  datatype)
    return data
...
def get_F1(data, preds):
    ...
    for i in range(0, len(data)):
        ...
        pred_triples, all_pred_triples, _ = get_pred_triples(preds[0][i], preds[1][i], preds[2][i], preds[3][i],
...
def write_test_res(src,data,preds,outfile):
    ...
    for i in range(0, len(src_lines)):
        ...
        pred_triples, _ ,out = get_pred_triples(preds[0][i], preds[1][i], preds[2][i], preds[3][i], preds[4][i],
                                        data[i].SrcWords)
        pred_triples_str = []
        # print(out)
        f = 1
...
if __name__ == "__main__":
    ...
    if job_mode == 'test':
        ...
        write_test_res(src_test_file, test_data,test_preds, os.path.join(trg_data_folder, 'test.out'))
